...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function connectWebSocket() {
    ...
    ws.onopen = () => {
      ...
      addLog('WEBSOCKET TELEMETRY LINK ESTABLISHED.', 'system');
    };
    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (msg.type === 'telemetry' || msg.type === 'init') {
          ...
          speakJarvisResponse(msg.response_text, msg.audio_url);
        }
      } catch (e) {
  ...
  async function executeCommand(text) {
    ...
    if (ws && ws.readyState === WebSocket.OPEN) {
      ...
      try {
        ...
        speakJarvisResponse(data.response_text, data.audio_url);
      } catch (e) {
        addLog(`COMMAND DISPATCH ERROR: ${e.message}`, 'system');
        setUIState('STANDBY');
      }
    }
  }
